...
class Product(list, Expression):
    ...
    """
    def __repr__(self):
        return "Product(%s)" % list.__repr__(self)  # data formatting
...
def do_multiply(expr1, expr2):
    ...
    * expr1 is a Product, and expr2 is a Product
    ...
    if isinstance(expr1, Sum) and isinstance(expr2, Sum):
        # return Sum(list(expr1.simplify()) + list(expr2.simplify()))
        for add1 in expr1:
            for add2 in expr2:
                sum.append(multiply(simplify_if_possible(add1), simplify_if_possible(add2)))
        return Sum(sum)
    elif isinstance(expr1, Sum) and isinstance(expr2, Product):
        for add1 in expr1:
            sum.append(multiply(simplify_if_possible(add1), simplify_if_possible(expr2)))
        return Sum(sum)
    elif isinstance(expr1, Product) and isinstance(expr2, Sum):
        for add2 in expr2:
            sum.append(multiply(expr1, add2))
        ...
        raise Exception, "Unknow type -- DO_MULTIPLY"
