...
def error_to_non_zero(error_rate):
    """
    convert error_rate to non_zero
    """
    doubt = 1e-4
    return min(max(error_rate, doubt), 1-doubt)
...
class BoostClassifier(Classifier):
    ...
    def classify(self, obj):
        ...
        for classifier, alpha in self.classifiers:
            hx += alpha * classifier.classify(obj)
    ...
    def update_weights(self, best_error, best_classifier):
        ...
        best_error = error_to_non_zero(best_error)

        for index in xrange(len(self.data)):
            datum = self.data[index]
            dweight = self.data_weights[index]
            if best_classifier.classify(datum) != self.standard.classify(datum):
                # wrong
                self.data_weights[index] = dweight * 0.5 / best_error
            else:
                # correct
                self.data_weights[index] = dweight * 0.5 / (1 - best_error)

    def data_and_weight(self):
        """
        Return a list of [data, weight]
        """
        return zip(self.data, self.data_weights)
