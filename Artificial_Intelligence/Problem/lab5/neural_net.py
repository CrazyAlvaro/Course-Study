...
class Weight(ValuedElement):
    ...
    def set_next_value(self,val):
        # print "curr: ", self.my_value, "next: ", val
        self.next_value = val
...
class Neuron(DifferentiableElement):
    ...
    def get_descendant_weights(self):
        ...
        if self.my_descendant_weights is None:
            ...
            for i in xrange(len(weights)):
                ...
                input = inputs[i]
                if not isinstance(input, Input):    # NOTE: input is a Neuron
    ...
    def compute_output(self):
        ...
        return 1.0 / float(1.0 + math.exp(-z))
    ...
    def compute_doutdx(self, elem):
        ...
        """
        # check to see if elem is direct weight
        descendant_weights = self.get_descendant_weights()

        # check to see if elem is indirect weight
        inputs = self.get_inputs()
        weights = self.get_weights()
        curr_output = self.output()
        perf_des = curr_output * (1 - curr_output)

        dout = 0
        for i in xrange(len(weights)):
            if elem.get_name() == weights[i].get_name():
                # direct weights
                dout += perf_des * inputs[i].output()
            elif self.isa_descendant_weight_of(elem, weights[i]):
                # indirect
                dout += perf_des * weights[i].get_value() * inputs[i].dOutdX(elem)
...
class PerformanceElem(DifferentiableElement):
    ...
    def output(self):
        ...
        returns: number (float/int)
        """
        result = -0.5 * ((self.my_desired_val - self.my_input.output()) ** 2)
        return result
...
def make_neural_net_basic():
    ...
    A = Neuron('A', [i1,i2,i0], [w1A,w2A,wA])   # NOTE: name, inputs, input_weights
    P = PerformanceElem(A, 0.0) # NOTE: input, desired_value
...
def make_neural_net_two_layer():
    ...
    # NOTE
    #  Neural Nets can be quite sensitive to initialization weight settings.
    # So order here really maters
    ...
    Neuron_A = Neuron('A', [i0, i1, i2], [wA, w1A, w2A])
    Neuron_B = Neuron('B', [i0, i1, i2], [wB, w1B, w2B])
    Neuron_C = Neuron('C', [i0, Neuron_A, Neuron_B], [wC, wAC, wBC])

    P = PerformanceElem(Neuron_C, 0.0)

    net = Network(P, [Neuron_A, Neuron_B, Neuron_C])
    return net
...
def make_neural_net_challenging():
    ...
    '''
    '''
    Depending on your architecture and your initial weights,
    your network may either easily learn patchy or get stuck in a local maximum.
    Does your network completely learn the dataset within 10000 iterations?
    If not, take a look at the weights output at the end of the 10000 iterations.
    Plot the weights in terms of a linear function on a 2D graph.
    Do the boundaries tell you why there might be a local maximum?

    I don't quite understand what would the 2D graph looks like, why it indicates a local maximum
    '''
    ...
    D = Neuron('D', [i0, i1, i2], [wD, w1D, w2D])
...
def train(network,
    ...
    while iteration < max_iterations:
        ...
        for datum in data:     # NOTE: for datum in data
            # set network inputs
            ...
            for w in network.weights:
                # print "increment: ", network.performance.dOutdX(w)
