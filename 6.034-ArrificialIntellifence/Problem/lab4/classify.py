...
def evaluate(factory, group1, group2, verbose=0):
    ...
    # NOTE: to use both as train and test
    ...
    if verbose >= 1:
        # len(group1) + len(group2) see NOTE above
...
def nearest_neighbors(distance_func, k=1):
    def nearest_neighbors_classifier(train):
        def classify_value(query):
            best_distance = INFINITY

            # NOTE: sort train data based on distance from query['votes']
            ordered = sorted(train, key=lambda x: distance_func(query['votes'], x['votes']))
            nearest = [x['party'] for x in ordered[:k]] # NOTE: get the first k data
            best_class = None
            best_count = 0
...
class CongressIDTree(object):
    def __init__(self, legislators, vote_meanings, disorder_func=None):
        ...
        if homog_test:
            ...
            # NOTE: Calculate which specific vote has the least disorder value
            for vote_index in xrange(len(legislators[0]['votes'])):
                # NOTE: vote yes, abstain or be absent, no
                for vote_value in [1, 0, -1]:
                    ...
                    # print "vote: %s \n vote_value: %s disord: %s" %\
                    #    (vote_info(vote_meanings[vote_index]), vote_value, disord)
