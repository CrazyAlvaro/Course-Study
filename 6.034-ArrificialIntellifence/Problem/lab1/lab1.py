...
ANSWER_5 = '0'  # the consequent is already present
...
# TODO: why need same identity, check forward_chain
...
grandparent_child = IF( AND('parent (?x) (?y)', 'parent (?y) (?z)'),
                        THEN('grandparent (?x) (?z)', 'grandchild (?z) (?x)'))

sibling = IF( AND('parent (?x) (?y)', 'parent (?x) (?z)', NOT('same-identity (?y) (?z)')),
...
cousin = IF( AND('sibling (?x) (?y)', 'parent (?x) (?a)', 'parent (?y) (?b)'),
             THEN('cousin (?a) (?b)', 'cousin (?b) (?a)'))
...
WHAT_I_FOUND_INTERESTING = 'All of them are very challenge and I had a lot of fun, found that write down the logic of the code really helps me \
                            understand the code. And also check the test case help me better understand how the function expect me to implement'
WHAT_I_FOUND_BORING = 'not at all'
