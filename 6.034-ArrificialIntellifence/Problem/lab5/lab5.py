...
# NOTE: To find out republican_newspaper_vote
# for classifier in boost_1796.classifiers:
    # print classifier,'\n'
...
def most_misclassified(classifier, n=5):
    ...
    """
    data_weights_list = classifier.data_and_weight()
    # sort by weight
    data_weights_list.sort(key=lambda tup: tup[1])
    data_weights_list.reverse() # sort() from small to large, need reverse

    # unzip list
    data, weights = zip(*data_weights_list)

    return map(legislator_info, data[0:n])
...
# NOTE: To find out republican_sunset_vote
...
# classifiers_for_best_ensemble = [ 'maj', 'dt', 'svml', 'svmr', 'svms'] 672
# classifiers_for_best_ensemble = [ 'maj', 'svml', 'svmr', 'svms'] 717
#classifiers_for_best_ensemble = [ 'maj'] 703
# classifiers_for_best_ensemble = [ 'knn'] 713
#classifiers_for_best_ensemble = [ 'dt'] 672
# classifiers_for_best_ensemble = [ 'svml'] 689
# classifiers_for_best_ensemble = [ 'svmr'] 717
# classifiers_for_best_ensemble = [ 'svms'] 703
# classifiers_for_best_ensemble = [ 'nb'] 727
classifiers_for_best_ensemble = [ 'maj', 'svml', 'svmr', 'svmp3', 'svms', 'nb']

if __name__ == "__main__":
    dataset = "breast-cancer"
    boost_learners = { key: learners[key] for key in classifiers_for_best_ensemble}
    ...
    print ("  accuracy: %.3f, brier: %.3f, auc: %.3f" %
           boosted_ensemble(dataset, boost_learners, DATASET_STANDARDS[dataset]))
